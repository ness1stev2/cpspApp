...
import { montoRangoValidator } from '../../../cpsp/validators/monto-validator';
...
@Component({
    ...
    standalone: true,
    imports: [ReactiveFormsModule, NgClass, NgIf, NgFor, DecimalPipe]
})
export class CreditoSimuladorComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private creditoServise: CpspService, private activateRoute: ActivatedRoute, private router: Router) { }
  ...
  onSubmit() {
    if (this.formSimulador.valid && this.credito) {
      ...
      //TODO La variable total a pagar es solo para saber el total que se pagaria incluyenfo los intereses
      /* let totalAPagar = 0; */
